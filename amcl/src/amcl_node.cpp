...
// Signal handling
#include <signal.h>  ///< 添加Linux软件中断,提供异常退出机制

#include "amcl/map/map.h"   ///< 解析地图
#include "amcl/pf/pf.h"     ///< 粒子滤波器
#include "amcl/sensors/amcl_odom.h"   ///< 里程计模型
#include "amcl/sensors/amcl_laser.h"  ///< 激光传感器模型
...
int main(int argc, char** argv)
{
  ...
  signal(SIGINT, sigintHandler);   ///<注册异常退出函数,退出时保存最后一次输出的定位点到参数服务器

  // Make our node available to sigintHandler
  amcl_node_ptr.reset(new AmclNode()); ///< boost智能指针构建了一个新的AmclNode对象
  ...
  ///< amcl根据输入参数来决定运行方式。从代码中，我们可以看到amcl支持数据回放的，运行时只需要通过参数"--run-from-bag"指定bag文件即可
        ...
        first_reconfigure_call_(true)       // 初次调用参数重配置
{
  ...
  ///< 对信号量configuration_mutex_加锁，防止与动态参数配置冲突
  boost::recursive_mutex::scoped_lock l(configuration_mutex_);

  // Grab params off the param server
  ///<"use_map_topic"是ROS导航栈在1.4.2版本新增的功能，amcl将订阅一个map主题来接收地图数据。否则通过请求map_server的服务来获取地图数据
  private_nh_.param("use_map_topic", use_map_topic_, false);  

  ///< 该参数为真时，amcl将只使用所订阅主题的第一帧地图数据进行定位，而不再更新地图。
  private_nh_.param("first_map_only", first_map_only_, false);

  double tmp;

  ///< 发布扫描数据和路径到可视化界面的最大频率，-1关闭此功能
  private_nh_.param("gui_publish_rate", tmp, -1.0);
  gui_publish_period = ros::Duration(1.0/tmp);

  ///<保存最近一次估计的位姿和协方差到参数服务器中的最大频率，即修改~initial_pos_*和~intial_cov*。-1关闭此功能
  private_nh_.param("save_pose_rate", tmp, 0.5); 
  save_pose_period = ros::Duration(1.0/tmp);

  private_nh_.param("laser_min_range", laser_min_range_, -1.0); ///< 激光扫描的最小距离，该值为-1时，amcl将使用激光传感器上报数据中的最小值。
  private_nh_.param("laser_max_range", laser_max_range_, -1.0); ///< 激光扫描的最大距离，该值为-1时，amcl将使用激光传感器上报数据中的最大值。
  private_nh_.param("laser_max_beams", max_beams_, 30); ///< 更新滤波器的时候，使用激光扫描束的间隔。一方面可以减少计算量，另一方面也是因为临近的激光数据之间也不是独立的。
  private_nh_.param("min_particles", min_particles_, 100); ///< 粒子滤波器使用的最小粒子数量。
  private_nh_.param("max_particles", max_particles_, 5000); ///< 粒子滤波器使用的最大粒子数量。
  private_nh_.param("kld_err", pf_err_, 0.01);   ///< 真实分布与估计分布之间的最大误差
  private_nh_.param("kld_z", pf_z_, 0.99);       ///< 上标准分位数（1-p），其中p是估计分布上误差小于kld_err的概率。 

  ///< 参考：https://blog.csdn.net/qq_29796781/article/details/80001355?utm_medium=distribute.pc_aggpage_search_result.none-task-blog-2~all~first_rank_v2~rank_v25-1-80001355.nonecase&utm_term=amcl%E5%8F%82%E6%95%B0%E8%AF%A6%E8%A7%A3
  private_nh_.param("odom_alpha1", alpha1_, 0.2); ///< 机器人运动模型中，旋转部分的旋转期望噪声
  private_nh_.param("odom_alpha2", alpha2_, 0.2); ///< 机器人运动模型的平移部分的旋转期望噪声
  private_nh_.param("odom_alpha3", alpha3_, 0.2); ///< 机器人运动模型的平移部分的平移期望噪声
  private_nh_.param("odom_alpha4", alpha4_, 0.2); ///< 机器人运动模型的旋转部分的平移期望噪声
  private_nh_.param("odom_alpha5", alpha5_, 0.2); ///< 平移相关的噪声参数(只适用于"omni"类型的里程计),该参数只对全向运动模型有用
  ...
  ///< 以下权重参考概率机器人
  private_nh_.param("laser_z_hit", z_hit_, 0.95);    ///< 传感器模型中的z_hit部分的混合权重
  private_nh_.param("laser_z_short", z_short_, 0.1); ///< 传感器模型中的z_short部分的混合权重
  private_nh_.param("laser_z_max", z_max_, 0.05);    ///< 传感器模型中的z_max部分的混合权重
  private_nh_.param("laser_z_rand", z_rand_, 0.05);  ///< 传感器模型中的z_rand部分的混合权重
  private_nh_.param("laser_sigma_hit", sigma_hit_, 0.2); ///< z_hit部分的高斯标准差：0.2m
  private_nh_.param("laser_lambda_short", lambda_short_, 0.1); ///< z_short部分的指数衰减系数
  private_nh_.param("laser_likelihood_max_dist", laser_likelihood_max_dist_, 2.0); ///< 似然场模型中，对障碍物膨胀的最大距离。默认2.0m
  std::string tmp_model_type;

  ///< 激光传感器模型，可以是"beam"、"likelihood_field"、"likelihood_field_prob"
  ...
  ///< 里程计模型类型，可以是"diff"、"omni"、"diff-corrected"、"omni-corrected"
  ...
  private_nh_.param("update_min_d", d_thresh_, 0.2); ///< 执行一次滤波器更新所需的最小位移:0.2m
  private_nh_.param("update_min_a", a_thresh_, M_PI/6.0); ///< 执行一次滤波器更新所需的最小转角:pi/6
  private_nh_.param("odom_frame_id", odom_frame_id_, std::string("odom"));      ///< 里程计坐标系。
  private_nh_.param("base_frame_id", base_frame_id_, std::string("base_link")); ///< 机器人底盘坐标系。
  private_nh_.param("global_frame_id", global_frame_id_, std::string("map"));   ///< 定位系统的全局坐标系

  ///< 重采样间隔，即至少进行了~resample_interval次的滤波器更新之后，才能进行一次重采样      
  ...
  ///< 推迟发布坐标变换的时间，to indicate that this transform is valid into the future
  double tmp_tol;
  private_nh_.param("transform_tolerance", tmp_tol, 0.1); 


  ///< 0.0(disabled):慢速平均权重滤波器(slow average weight filter)的指数衰减速率，用于确定添加随机位姿的时机，以达到recover的目的。一般可以取0.001
  private_nh_.param("recovery_alpha_slow", alpha_slow_, 0.001);

  ///< 0.0(disabled):快速平均权重滤波器(fast average weight filter)的指数衰减速率，用于确定添加随机位姿的时机，以达到recover的目的。一般可以取0.1
  private_nh_.param("recovery_alpha_fast", alpha_fast_, 0.1);

  ///< 设置为false，将不发布从全局坐标到里程计坐标的变换关系
  ...
  ///< 注册三个服务，其中：
  ///< global_localization用于获取机器人的全局定位，
...
void AmclNode::savePoseToServer()
{
  ...
  private_nh_.setParam("initial_cov_aa", 
                                  last_published_pose.pose.covariance[6*5+5]); ///< 初始方向角方差(a*a)，用作初始位姿的高斯分布的协方差矩阵
}


 ///< 用来从参数服务器上获取机器人的初始位姿和位姿误差的协方差矩阵
void AmclNode::updatePoseFromServer()
{
  init_pose_[0] = 0.0;///< 初始位姿中心的x分量，用作初始位姿的高斯分布的均值
  init_pose_[1] = 0.0;///< 初始位姿中心的y分量，用作初始位姿的高斯分布的均值
  init_pose_[2] = 0.0;///< 初始方向角，用作初始位姿的高斯分布的均值
  init_cov_[0] = 0.5 * 0.5;///< 初始位姿方差(x*x)，用作初始位姿的高斯分布的协方差矩阵：0.5 * 0.5
  init_cov_[1] = 0.5 * 0.5;///< 初始位姿方差(y*y)，用作初始位姿的高斯分布的协方差矩阵：0.5 * 0.5
  init_cov_[2] = (M_PI/12.0) * (M_PI/12.0);//< 初始方向角方差(a*a)，用作初始位姿的高斯分布的协方差矩阵:(M_PI/12.0) * (M_PI/12.0)
